...
const App = () => {
  const TOKEN =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJJZCI6IjY1NzgyZTVlNjkxMDkxMWQyOTcwNzhlMiIsIm5hbWUiOiJDb2Rpb3RpYyIsInVzZXJUeXBlIjoiU1VQRVJfQURNSU4iLCJ0b2tlblR5cGUiOiJMT0dJTiIsImlhdCI6MTcxNjg5NTE5NSwiZXhwIjoxNzE3MTYxNTk1fQ.nfOoaEqGj6fXA6Hyc7Q-yk6uIcghVWMH7hA4z00zNWs";
  ...
  const onDragEnd = (result) => {
    ...
    if (startColumn === finishColumn) {
      ...
      // Un Comment this for using via the api
  ...
  const getTotalBoardsWidth = () => {
    return state.columnOrder.length * 366;
  };
  ...
  return (
    <div
      className="flex gap-[20px] mt-[50px] ml-[50px] mr-[100px]"
      style={{
        // width: 5000,
              ...
              >
                <div className="flex gap-[4px] items-center relative">
                  <span
                    ...
                    className={`transition-all hover:bg-gray-200 rounded flex-shrink-0 overflow-hidden cursor-pointer p-1 ${
                      collapse && "rotate-90 absolute left-0"
                    }`}
                  >
                ...
                */}
                  <span
                    className={`${
                      collapse
                        ? "block absolute top-16 left-2 transform -translate-y-1/2 -translate-x-1/2 rotate-90 font-bold"
                        : "block"
                    } whitespace-nowrap`}
                  >
                    {column.title}
                        ...
                        {column.taskIds.length >= 10 ? (
                          column.taskIds.length === totalItem ? (
                            ...
                            <div className="flex justify-center flex-col items-center">
                              ...
                              Showing {column.taskIds.length} item of{" "}
                              {totalItem}
                            </div>
                          )
                        ) : null}
